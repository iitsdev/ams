...
const props = defineProps<{
    audit: { id: number; status: string; location?: { id: number; name: string } | null; created_at: string; closed_at?: string | null; notes?: string | null }
    entries: Array<any>
    ...
    const detect = async () => {
        ...
        try {
            ...
            if (barcodes && barcodes.length && !scanningCooldown) {
                ...
                setTimeout(() => { scanningCooldown = false }, 800)
            }
        } catch {
...
watch(selectedDeviceId, async (id, oldId) => {
    if (id === oldId) return
    try { localStorage.setItem('audit.cameraDeviceId', id || '') } catch { }
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500">Status: {{ props.audit.status }} | Location: {{
                    props.audit.location?.name || 'All' }}</p>
                <p v-if="lastScannedTag" class="mt-1 text-sm text-green-700">Last scanned: {{ lastScannedTag }}</p>
            </div>
            <div class="flex items-center gap-3">
                <select v-model="foundLocationId" class="border rounded px-2 py-1">
                ...
                <Button v-if="props.audit.status === 'open'" variant="secondary" @click="fetchVariance">Compute
                    Variance</Button>
        ...
        <div class="p-4 border rounded-md">
            <div class="flex items-end gap-3">
                <div class="flex-1">
                    ...
                    <Input ref="scanInputRef" v-model="scanCode" @keyup.enter="scan" class="mt-1 w-full"
            ...
            <div class="mt-4 flex items-center gap-3" v-if="props.audit.status === 'open'">
                ...
                <div v-if="cameraEnabled" class="flex items-center gap-2">
                    ...
                    <select v-model="selectedDeviceId" class="border rounded px-2 py-1">
                        <option v-for="d in devices" :key="d.deviceId" :value="d.deviceId">{{ d.label || 'Camera' }}
                        </option>
                    </select>
                </div>
                <Button v-if="cameraEnabled && canToggleTorch()" variant="outline" @click="toggleTorch">{{ torchOn ?
                    'Torch Off' : 'Torch On' }}</Button>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full border">
                <thead>
                    <tr class="bg-gray-50">
                        <th class="px-3 py-2 text-left border">Asset Tag</th>
                        <th class="px-3 py-2 text-left border">Name</th>
                        <th class="px-3 py-2 text-left border hidden md:table-cell">Category</th>
                        <th class="px-3 py-2 text-left border">Recorded Location</th>
                        <th class="px-3 py-2 text-left border">Found Location</th>
                        <th class="px-3 py-2 text-left border hidden md:table-cell">Scanned By</th>
                        <th class="px-3 py-2 text-left border">Scanned At</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="e in list" :key="e.id"
                        :class="['hover:bg-gray-50', lastScannedId === e.id ? 'bg-green-50' : '']">
                        <td class="px-3 py-2 border">{{ e.asset.asset_tag }}</td>
                        <td class="px-3 py-2 border">{{ e.asset.name }}</td>
                        <td class="px-3 py-2 border hidden md:table-cell">{{ e.asset.category || '—' }}</td>
                        <td class="px-3 py-2 border">{{ e.asset.location || '—' }}</td>
                        <td class="px-3 py-2 border">{{ e.found_location || '—' }}</td>
                        <td class="px-3 py-2 border hidden md:table-cell">{{ e.scanned_by || '—' }}</td>
                        <td class="px-3 py-2 border">{{ e.scanned_at }}</td>
                    </tr>
                </tbody>
            </table>
        </div>

        <div v-if="showVariance && variance" class="p-4 border rounded-md space-y-4">
            <h2 class="text-lg font-semibold">Variance</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <h3 class="font-medium mb-2">Missing</h3>
                    <ul class="list-disc ml-5 space-y-1">
                        <li v-for="a in variance.missing" :key="a.id">{{ a.asset_tag }} — {{ a.name }} ({{
                            a.location?.name || '—' }})</li>
                    </ul>
                </div>
                <div>
                    <h3 class="font-medium mb-2">Extra</h3>
                    <ul class="list-disc ml-5 space-y-1">
                        <li v-for="a in variance.extra" :key="a.id">{{ a.asset_tag }} — {{ a.name }} ({{
                            a.location?.name || '—' }})</li>
                    </ul>
                </div>
                <div>
                    <h3 class="font-medium mb-2">Moved</h3>
                    <ul class="list-disc ml-5 space-y-1">
                        <li v-for="e in variance.moved" :key="e.id">
                            {{ e.asset.asset_tag }} — {{ e.asset.name }}: recorded {{ e.asset.location?.name || '—' }},
                            found {{ e.found_location?.name || '—' }}
                        </li>
                    </ul>
                </div>
            </div>
